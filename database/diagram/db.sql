...
CREATE TABLE IF NOT EXISTS `db`.`gazette` (
  ...
  `idfamily` INT NOT NULL,
  `idobject` INT NULL,
  `type` TINYINT NOT NULL DEFAULT 1 COMMENT '1: 16 pages\n2: 32 pages\n…',
...
CREATE TABLE IF NOT EXISTS `db`.`song` (
  ...
  PRIMARY KEY (`idsong`),
  INDEX `fk_song_text1_idx` (`idtext` ASC) VISIBLE,
  CONSTRAINT `fk_song_text1`
    FOREIGN KEY (`idtext`)
    REFERENCES `db`.`text` (`idtext`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
