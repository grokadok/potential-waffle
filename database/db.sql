...
CREATE TABLE IF NOT EXISTS `db`.`recipient` (
  ...
  INDEX `fk_recipient_user1_idx` (`added_by` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `db`.`gazette` (
  ...
  `date` DATE NOT NULL,
  `recipient_idrecipient` INT NOT NULL,
  PRIMARY KEY (`idgazette`),
  INDEX `fk_gazette_recipient1_idx` (`recipient_idrecipient` ASC) VISIBLE,
  CONSTRAINT `fk_gazette_recipient1`
    FOREIGN KEY (`recipient_idrecipient`)
    REFERENCES `db`.`recipient` (`idrecipient`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
